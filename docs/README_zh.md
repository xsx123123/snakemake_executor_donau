...
### 1. 执行器系统日志 (Workdir)
调度行为日志现在会直接生成在您的工作目录下，并提供更专业的排版：
- **路径**: 自动在 workflow 的当前工作目录下创建（尊重 Snakefile 的 `workdir` 或 `--directory` 参数）。
- **内容**: 
  - **彩色排版**: 采用现代风格，不同级别日志颜色分明。
  - **精准溯源**: 每条日志包含精确到毫秒的时间戳，以及输出该日志的 **代码模块:函数:行号**，极大方便调试。
  - **详细记录**: 记录 UUID、执行的完整 Shell 命令 (`dsub`) 以及调度器的原始响应。
